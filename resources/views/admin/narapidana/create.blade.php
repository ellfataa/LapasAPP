<x-app-layout>
    ...
    <div x-data="{ sidebarOpen: false, showAlert: {{ $errors->any() ? 'true' : 'false' }} }"
        ...
        <main class="flex-1 flex flex-col bg-slate-50 min-w-0 p-4 sm:p-6 lg:p-8">
            ...
            <div class="bapas-admin-header flex items-center gap-4 bg-slate-50 mb-8">
                ...
                <h2 class="font-bold text-xl sm:text-2xl md:text-3xl text-slate-900 leading-tight tracking-tight">
                    Tambah Akun Klien/Narapidana
                </h2>
            </div>

            <div class="max-w-3xl bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8 mx-auto w-full">
                <div class="border-b border-slate-100 pb-4 mb-6">
                    <h3 class="text-lg font-bold text-slate-800">Formulir Tambah Klien/Narapidana</h3>
                    <p class="text-sm text-slate-500 mt-1">Pastikan data NIK/Registrasi yang diinput belum pernah terdaftar di sistem.</p>
                </div>

                <form action="{{ route('admin.narapidana.store') }}" method="POST" class="space-y-5" autocomplete="off">
                    @csrf
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Nama Lengkap Klien/Narapidana</label>
                        <input type="text" name="nama" value="{{ old('nama') }}" required pattern="[a-zA-Z\s]+" title="Nama hanya boleh berisi huruf dan spasi" placeholder="Masukkan nama Klien" class="block min-h-[48px] w-full rounded-xl border-slate-300 px-4 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="contoh: klien@gmail.com" class="block min-h-[48px] w-full rounded-xl border-slate-300 px-4 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Password Akun (Akses Awal)</label>
                    ...
                    <div class="pt-6 mt-6 flex items-center justify-end gap-3 border-t border-slate-100">
                        ...
                        <button type="submit" class="inline-flex min-h-[48px] items-center justify-center px-8 py-2.5 rounded-xl bg-green-700 text-white font-bold text-sm hover:bg-green-800 transition-colors shadow-md">Daftarkan Klien</button>
                    </div>
                </form>
            </div>
        </main>
        ...
        <div x-cloak x-show="showAlert" class="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/65 px-4 backdrop-blur-sm">
            ...
            <div class="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 sm:p-8 text-center shadow-2xl">
                ...
                <h3 class="font-bold text-xl text-slate-900 mb-3">Gagal Mendaftarkan Klien</h3>
