    ...
    <div x-data="{ sidebarOpen: false, showImportModal: false, showAlert: {{ session('success') || $errors->any() ? 'true' : 'false' }}, fileName: '', fileSelected: false }"
        ...
        <main class="flex-1 flex flex-col min-w-0">
            ...
            <div class="p-4 sm:p-6 lg:p-8 space-y-6 flex-1 overflow-y-auto">
                ...
                <div class="md:hidden bg-white border-b border-slate-200 px-4 py-3 flex items-center shadow-sm rounded-xl mb-4">
                ...
                <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 w-full sm:w-auto xl:justify-end">
                        ...
                        <a href="{{ route('admin.narapidana.create') }}" class="w-full sm:w-auto min-h-[44px] bg-indigo-700 hover:bg-indigo-800 text-white font-bold py-2.5 px-4 sm:px-5 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm text-center">
                            ...
                            <span class="whitespace-nowrap">Tambah Klien Manual</span>
                        </a>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                    <form method="GET" action="{{ route('admin.narapidana.index') }}" class="flex flex-col sm:flex-row gap-4">
                        <div class="relative flex-1">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari berdasarkan Nama Lengkap atau Nomor Induk (NIK/Nomor Registrasi)..." class="block min-h-[48px] w-full pl-12 pr-4 rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm shadow-sm transition">
        ...
        <div x-cloak x-show="showImportModal" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm px-4">
            ...
            <div class="relative z-10 w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden transform transition-all">
                ...
                <div class="bg-emerald-700 px-6 py-4 flex justify-between items-center text-white">
                    <h3 class="font-bold text-lg">Import Data Klien via Excel/CSV</h3>
                    <button @click="showImportModal = false" class="text-emerald-200 hover:text-white transition">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>

                <form action="{{ route('admin.narapidana.import') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                    @csrf
                    ...
                    <div class="bg-emerald-50 text-emerald-800 p-4 rounded-xl text-sm border border-emerald-100">
                        ...
                        <ul class="list-disc list-inside space-y-1 text-xs">
                            ...
                            <li>Kolom B (Kolom 2): <strong>NIK/Nomor Registrasi *Bisa dikosongkan</strong></li>
                            <li>Jika Nama sudah ada di sistem, maka yang terupdate hanya Nomor Registrasi.</li>
                            <li>Jika Nomor Registrasi kosong, sistem akan membuatkan nomor otomatis.</li>
                            <li>Password default Klien: <span class="bg-emerald-200 text-emerald-900 px-1.5 py-0.5 rounded font-mono font-bold">bapas123</span></li>
                        </ul>
                    </div>
                    ...
                    <div class="relative border-2 border-dashed rounded-xl p-6 text-center transition-colors cursor-pointer group"
                        ...
                        <div x-show="!fileSelected" class="flex flex-col items-center pointer-events-none">
                            ...
                            <span class="block text-sm font-bold text-slate-800 mb-1">Pilih File Dokumen Klien</span>
