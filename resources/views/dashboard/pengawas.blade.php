    ...
    <div class="min-h-screen bg-slate-100 py-6 sm:py-10" x-data="{ showModal: false, imgSrc: '', showAlert: {{ session('success') || $errors->any() ? 'true' : 'false' }} }">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:space-y-8 sm:px-6 lg:px-8">
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-blue-800 bg-blue-900 px-5 py-5 sm:px-7 sm:py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm leading-relaxed text-blue-100">Lengkapi capaian kinerja. Pengisian hanya dapat dilakukan 1 kali per bulan.</p>
                        </div>
                    </div>
                    ...
                    @endphp
                    <div class="bg-blue-950 px-4 py-2 rounded-xl border border-blue-800 shadow-inner">
                        <span class="block text-[11px] text-blue-300 font-bold uppercase tracking-widest mb-0.5">Periode Penilaian</span>
                ...
                <div class="p-5 sm:p-7 lg:p-8">
                    ...
                    <script>
                        ...
                        // PERBAIKAN: Memindahkan data array JSON ke Script Global agar tidak memecah struktur HTML
                        window.rekapTahunanData = {!! json_encode($rekapPengawasanTahunan) !!};
                    </script>
                    ...
                    <form action="{{ route('kinerja-pk.store') }}" method="POST" x-data="{
                        ...
                        currentMonth: {{ date('n') }},
                        viewMonth: '{{ date('n') }}',
                        rekapTahunan: window.rekapTahunanData, // PERBAIKAN: Memanggil dari window
                        ...
                        // Kalkulasi Aktual (Selalu Bulan Berjalan)
                        get pengawasanPersenReal() {
                            let aktual = this.rekapTahunan[this.currentMonth] || 0;
                            if(this.totalKlien > 0) return (aktual / this.totalKlien) * 100;
                            return 0;
                        },
                        ...
                        init() {
                            ...
                            if (serverDraft) {
                                ...
                                if (this.skorDihitung) this.hitungSkor();
                            }
                        ...
                        saveDraft() {
                            ...
                            this.draftTimeout = setTimeout(() => {
                                const draftJSON = JSON.stringify({ klien: this.klien, skorDihitung: this.skorDihitung });
                                        ...
                                        <div class="max-h-[300px] overflow-y-auto custom-scrollbar divide-y divide-slate-100">
                                            @forelse($daftarKlienSaya as $klien)
                                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 hover:bg-slate-50 transition">
                                                    ...
                                                    <select name="pembimbingan_klien[{{ $klien->id }}]" x-model="klien['{{ $klien->id }}']" required class="block w-full sm:w-[190px] rounded-lg border-slate-300 py-2 px-3 text-sm shadow-sm cursor-pointer focus:ring-blue-500 focus:border-blue-500 transition-colors border" :class="{'bg-emerald-50 text-emerald-700 font-bold border-emerald-300': klien['{{ $klien->id }}'] === 'bekerja', 'bg-red-50 text-red-700 font-medium border-red-300': klien['{{ $klien->id }}'] === 'belum', 'bg-white text-slate-500 border-slate-300': klien['{{ $klien->id }}'] === '' || klien['{{ $klien->id }}'] === undefined }">
                                    ...
                                    <div class="bg-indigo-900 rounded-xl p-6 text-white shadow-md flex-1 flex flex-col justify-center relative overflow-hidden">
                                        ...
                                        <div x-show="skorDihitung" x-transition class="grid grid-cols-3 gap-2 text-center text-xs border-t border-indigo-700/50 pt-4 relative z-10" style="display: none;">
                                            <div class="bg-indigo-800/50 rounded-lg p-2 flex flex-col items-center justify-center">
                                                <span class="block text-indigo-300 mb-0.5 leading-none">Total</span><span class="font-bold text-sm leading-none">{{ $totalKlienSaatIni }}</span>
                                            </div>
                                            <div class="bg-emerald-800/30 rounded-lg p-2 border border-emerald-500/30 flex flex-col items-center justify-center">
                                                <span class="block text-emerald-300 mb-0.5 leading-none">Bekerja</span><span class="font-bold text-sm text-emerald-400 leading-none" x-text="jmlBekerja"></span>
                                            </div>
                                            <div class="bg-red-800/30 rounded-lg p-2 border border-red-500/30 flex flex-col items-center justify-center">
                                                <span class="block text-red-300 mb-0.5 leading-none">Belum</span><span class="font-bold text-sm text-red-400 leading-none" x-text="jmlBelumBekerja"></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <!-- KARTU 3: PENGAWASAN (OTOMATIS SERVER-SIDE) -->
                        <article class="mb-8 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 transition duration-200 hover:border-blue-200 hover:shadow-sm">
                            <div class="border-b border-slate-200 bg-white px-5 py-4 sm:px-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <h4 class="flex items-center gap-3 text-base font-bold text-slate-900 sm:text-lg">
                                ...
                                <!-- FITUR FILTER BULAN SIMULASI -->
                                <div class="flex items-center gap-2">
                                    <label class="text-xs font-bold text-slate-500">Cek Bulan:</label>
                                    <select x-model="viewMonth" class="text-sm font-bold text-blue-800 bg-white border-blue-200 rounded-lg py-1.5 pl-3 pr-8 shadow-sm cursor-pointer hover:border-blue-400 focus:ring-blue-500">
                            ...
                            <div class="grid grid-cols-1 items-start gap-5 p-5 sm:p-6 lg:grid-cols-3">
                                <div>
                                    ...
                                    <input type="text" :value="totalKlien" readonly class="block min-h-[44px] w-full cursor-not-allowed rounded-xl border-slate-200 bg-slate-100 px-4 text-base font-bold text-slate-600 shadow-sm focus:ring-0">
                                </div>
                                <div>
                                    <label class="mb-2 block text-sm font-bold text-slate-800">Klien Berhasil Absen</label>
                                    <input type="text" :value="pengawasanBerhasil" readonly class="block min-h-[44px] w-full cursor-not-allowed rounded-xl border-slate-200 bg-slate-100 px-4 text-base font-bold text-emerald-700 shadow-sm focus:ring-0">
                                </div>
                                <div>
                                    <label class="mb-2 flex justify-between text-sm font-bold text-slate-800">
                                        Skor Kinerja Pengawasan
                                        <span x-show="viewMonth != currentMonth" class="text-[10px] text-red-500 font-medium italic mt-0.5">*Simulasi (Bukan Skor Real)</span>
                                        <span x-show="viewMonth == currentMonth" class="text-[10px] text-emerald-600 font-bold italic mt-0.5">*Bulan Berjalan (Skor Real)</span>
                                    </label>
                                    <div class="relative">
                                        <input type="text" :value="pengawasanPersenView" readonly class="block min-h-[44px] w-full cursor-not-allowed rounded-xl border-slate-300 bg-slate-200 px-4 font-extrabold text-blue-800 shadow-sm focus:ring-0" :class="{'border-red-300 bg-red-50 text-red-700' : viewMonth != currentMonth}">
                                        <span class="absolute right-3 top-2.5 font-bold text-slate-600" :class="{'text-red-700' : viewMonth != currentMonth}">%</span>
                                    </div>
                                </div>
                            </div>
                        </article>
                        ...
                        <div class="mb-6 flex flex-col items-stretch justify-between gap-5 rounded-2xl border border-blue-200 bg-blue-50 p-5 sm:p-6 md:flex-row md:items-center">
                            <div class="text-center md:text-left">
                                ...
                                <p class="text-sm font-medium leading-relaxed text-blue-800">Nilai dihitung berdasarkan 3 kategori di atas pada <b>Bulan Berjalan</b>.</p>
                            </div>
                ...
                <div class="custom-scrollbar overflow-x-auto">
                    <table class="w-full min-w-[900px] table-fixed text-left text-slate-700">
                        <thead class="border-b border-slate-200 bg-slate-100 text-sm font-semibold text-slate-700">
                            <tr>
                                <th class="w-40 border-r border-slate-200 px-6 py-4 text-xs font-bold uppercase tracking-wide">Periode</th>
                                <th class="border-r border-slate-200 px-6 py-4 text-xs font-bold uppercase tracking-wide">Rincian Kinerja 3 Kategori</th>
                                <th class="w-44 px-6 py-4 text-center text-xs font-bold uppercase tracking-wide">Predikat Akhir</th>
                            </tr>
                        </thead>
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                                            ...
                                            @foreach(['litmas', 'pembimbingan', 'pengawasan'] as $kat)
                                                ...
                                                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
                                                    <div class="flex flex-col gap-2 mb-3">
                                                        <p class="text-[11px] font-extrabold uppercase tracking-widest text-slate-500">{{ $kat }}</p>
                                                        <span class="text-lg font-extrabold text-slate-800 leading-none">
                                                            {{ $kinerja->{$kat.'_berhasil'} }}
                                                            <span class="text-sm font-medium text-slate-400">/ {{ $kinerja->{$kat.'_kuota'} }}
                                                                @if($kat === 'litmas')
                                                                    Jumlah Litmas
                                                                @elseif($kat === 'pembimbingan')
                                                                    Klien Berhasil Kerja
                                                                @elseif($kat === 'pengawasan')
                                                                    Klien Berhasil Absen
                                                                @endif
                                                            </span>
                                                        </span>
                                                    </div>
                                                    <div class="flex items-center justify-between gap-3">
                                                        <div class="h-2 flex-1 overflow-hidden rounded-full bg-slate-200">
                                                            <div class="h-full rounded-full bg-blue-600" style="width: {{ $lebarProgress }}%"></div>
                                                        </div>
                                                        <span class="text-xs font-bold text-blue-800">{{ number_format($persenKat, 1) }}%</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
                            ...
                            @empty
                                <tr><td colspan="3" class="px-6 py-14 text-center text-slate-500">Belum ada riwayat kinerja yang tersimpan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="flex items-center gap-4 border-b border-slate-200 bg-white px-5 py-5 sm:px-7 sm:py-6">
                    ...
                    <div>
                        <h3 class="text-lg font-bold leading-snug text-slate-900 sm:text-xl">Filter Cari Absensi/Laporan Wajib Klien Saya</h3>
                ...
                <div class="p-5 sm:p-7 lg:p-8">
                    <form method="GET" action="{{ route('dashboard.pengawas') }}" class="grid grid-cols-1 items-end gap-5 md:grid-cols-12">
                        <div class="md:col-span-5">
                            <label for="search" class="mb-2 block text-sm font-bold text-slate-800 sm:text-base">Cari Nama/Nomor Induk (NIK/No Reg)</label>
                            <input type="text" id="search" name="search" value="{{ request('search') }}" placeholder="Ketik nama/NIK/No Reg klien..." class="block min-h-[48px] w-full rounded-xl border-slate-300 bg-white px-4 py-3 text-base text-slate-900 shadow-sm transition hover:border-slate-400 focus:border-blue-700 focus:ring-blue-700">
                        </div>
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="flex flex-col items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-5 sm:flex-row sm:items-center sm:px-7 sm:py-6">
                    <h3 class="flex items-center gap-4 text-lg font-bold leading-snug text-slate-900 sm:text-xl">
                        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-900 ring-1 ring-inset ring-blue-100"><svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg></span>
                        Daftar Absensi/Laporan Wajib Klien Saya
                    </h3>
                    ...
                    <table class="w-full min-w-[880px] text-left text-slate-700">
                        <thead class="border-b border-slate-200 bg-slate-100">
                            <tr>
                                ...
                                <th scope="col" class="w-64 px-6 py-4 text-xs font-bold uppercase tracking-wide text-slate-700">Nama Klien</th>
